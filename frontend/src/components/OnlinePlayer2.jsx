    ...
    <div className='flex sm:flex-col bg-[#bbada0] w-[300px] h-[250px] sm:w-[300px] sm:h-[500px] mx-0 sm:mx-15 items-center gap-10 rounded-lg'>
      {
        ...
        <div className='flex sm:flex-col sm:gap-10 items-center w-full '>
            ...
            <Button  className='w-20 sm:w-auto' disabled={!isConnected || gameState.playerNumber != 2 || !playersCategory[1]} onClick={()=>{selectCategory(selectedcategory);setPlayersCategory({...playersCategory, 2:selectedcategory});setPlayersReady({...PlayersReady, 2:true});setSelectedCategory('');moveSound.play();}}>Ready</Button>
          </div>
        </div>
      }
    </div>
